...
Переключатели по умолчанию находятся в [`Core/Inc/experiment_config.h`](Core/Inc/experiment_config.h):
...
Основные настройки изменяются в [`Core/Inc/experiment_config.h`](Core/Inc/experiment_config.h). Расширенные переключатели, отмеченные как `main.c`, остаются в [`Core/Src/main.c`](Core/Src/main.c). После изменения сохраните файл, пересоберите проект и прошейте плату.
...
| `SCHEDULER_MODE` (`main.c`)                                     | Режим ожидания интегрированного планировщика: busy-polling или WFI. Профили 3 и 4 всегда используют busy-polling. |
| `ENABLE_SYNTH_IMU`, `ENABLE_SYNTH_LIDAR`, `ENABLE_SYNTH_CAMERA` (`main.c`) | Включение синтетических задач интегрированного режима.                                             |
| `ENABLE_REAL_TAU1`, `ENABLE_REAL_TAU2` (`main.c`)               | Включение реальных HC-SR04 и DHT11; для запуска без датчиков оставьте `0`.                                        |
