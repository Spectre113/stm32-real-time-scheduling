...
\begin{figure*}[!t]
	...
	\caption{EDF execution scenario for the task set in Table~\ref{tab:superloop_tasks}. Hatching indicates a preempted job awaiting resumption.}
...
In Fig.~\ref{fig:chunked_edf}, $J_2^2$ is released while $J_1^1$ is executing its current chunk. Although $J_2^2$ has the earlier absolute deadline, preemption occurs only at the end of the chunk.

\begin{figure*}[!b]
	\centering
	\includegraphics[width=0.7\textwidth]{chunked_edf_execution_schedule.png}
