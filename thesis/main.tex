...
Figure~\ref{fig:edf_schedule} illustrates EDF scheduling for the task set in Table~\ref{tab:superloop_tasks}.
...
In Fig.~\ref{fig:chunked_edf}, $J_1^1$ executes a chunk from $t=19$ ms to $t=22$ ms. Job $J_2^2$ is released at $t=20$ ms, but the scheduler recheck the ready jobs only at $t=22$ ms. Since $J_2^2$ has the earlier absolute deadline, it is selected at the next chunk boundary.
