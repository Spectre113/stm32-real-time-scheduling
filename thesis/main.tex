...
\item then we implement chunked Earliest Deadline First (EDF) scheduler, and finally
\item we evaluate their performance on a real STM32 hardware.
\end{itemize}
...
\label{EDF_subsection}

Unlike Super Loop, Earliest Deadline First (EDF) selects jobs according
to their absolute deadlines.
...
Fig.~\ref{fig:edf_schedule} illustrates EDF scheduling for the task set in Table~\ref{tab:superloop_tasks}.

\begin{figure*}[!t]
	\centering
	\includegraphics[width=0.9\textwidth]{edf_execution_schedule.png}
	\caption{EDF execution scenario for the task set in
		Table~\ref{tab:superloop_tasks}. Hatching indicates a preempted
...
In the STM32 implementation, synthetic jobs execute in chunks of maximum duration $\Delta$, with job selection repeated after each chunk. An earlier-deadline job released during a chunk may therefore wait until the chunk completes. Sensor jobs use separate staged handlers.

For illustration, Fig.~\ref{fig:chunked_edf} uses $\Delta=4$ ms. Job $J_2^2$ is released at $t=20$ ms and preempts $J_1^1$ at the next chunk boundary $t=22$ ms. Thus, the preemption delay is $\delta=2$ ms. The final chunk of $J_2^2$ lasts $2$ ms, since the preceding two chunks account for $8$ ms of its execution time $C_2=10$ ms.

\begin{figure*}[!b]
	\centering
	\includegraphics[width=0.75\textwidth]{chunked_edf_execution_schedule.png}
	\caption{Chunk-bounded preemption in the EDF implementation.}
	\label{fig:chunked_edf}
\end{figure*}

Ignoring scheduling overhead, the delay until the next chunk boundary satisfies $0 \leq \delta \leq \Delta$.

Algorithm~\ref{alg:chunked_edf} summarizes chunked EDF for synthetic jobs, assuming first releases at $t=0$ as in the example. The set $\mathcal{R}$ contains jobs admitted for execution but not yet completed. The flag $active_i$ is true while a job of $\tau_i$ belongs to $\mathcal{R}$, including before its first execution. Equal deadlines are resolved by task order. \textsc{ExecuteChunk} executes at most $\Delta$ of the selected job's remaining execution and preserves its progress.

\begin{algorithm}[!t]
	\caption{Chunked EDF Scheduler}
	\label{alg:chunked_edf}
	\begin{algorithmic}[1]
		
		\Function{ChunkedEDF}{$\mathcal{T}, \Delta$}
		\State $\mathcal{R} \gets \emptyset$
		
		\ForAll{$\tau_i \in \mathcal{T}$}
		\State $t_i^{next} \gets 0$
		\State $active_i \gets false$
		\EndFor
		...
		\ForAll{$\tau_i \in \mathcal{T}$}
		\If{$\neg active_i \land t^{cur} \geq t_i^{next}$}
		\State $j \gets 1+t_i^{next}/T_i$
		\State $R_i^j \gets t_i^{next}$
		\State $d_i^j \gets R_i^j+D_i$
		\State $\mathcal{R} \gets \mathcal{R} \cup \{J_i^j\}$
		\State $active_i \gets true$
		\EndIf
		\EndFor
		
		\If{$\mathcal{R} \neq \emptyset$}
		\State $J_i^j \gets \Call{SelectEDF}{\mathcal{R}}$
		\Comment{Earliest deadline}
		
		\State \Call{ExecuteChunk}{$J_i^j,\Delta$}
		
		\If{\Call{Completed}{$J_i^j$}}
		...
		\State $\mathcal{R} \gets \mathcal{R} \setminus \{J_i^j\}$
		\State $active_i \gets false$
