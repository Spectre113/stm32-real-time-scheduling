...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\columnwidth]{stm32_hardware_platform.jpg}
...
\begin{itemize}
  \item A \emph{schedule} specifies which job executes on the processor at each time instant, or whether the processor executes no job.
  \item \emph{Preemption} interrupts a job before completion to execute another job. The interrupted job may resume later.
...
The considered Super Loop scheduler repeatedly scans tasks in a fixed order and executes each selected job non-preemptively. To illustrate its behavior, let us consider the two periodic tasks in Table~\ref{tab:superloop_tasks}.
...
\begin{figure*}[tb]
  \centering
  \includegraphics[width=0.8\textwidth]{superloop_execution_schedule.png}
...
Let $t^{cur}$ denote the current system time and $t_i^{next}$ the next scheduled release time of task $\tau_i$. During execution of $J_i^j$, the release variable $t_i^{next}$ retains $R_i^j$. At completion, $t^{cur}=f_i^j$, and a deadline miss is recorded if $f_i^j>d_i^j$. The scheduler then advances $t_i^{next}$ by $T_i$. While $t_i^{next}<t^{cur}$, that release is counted as skipped and $t_i^{next}$ is advanced by $T_i$ again. A release at $t_i^{next}=t^{cur}$ is retained.
...
The scheduling logic illustrated in Fig.~\ref{fig:superloop_schedule} is summarized in Algorithm~\ref{alg:superloop}.
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]
		...
		\State \Call{RecordSkip}{$t_i^{next}$}
		\State $t_i^{next} \gets t_i^{next} + T_i$
		\EndWhile
...
\todo{AB2VT: To describe pseudo-code of EDF}
...
We consider preemptive EDF, which dynamically selects the ready job with the earliest absolute deadline $d_i^j$ (Section~\ref{sec:system_model}).
