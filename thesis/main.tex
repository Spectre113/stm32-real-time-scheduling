...
The STM32 implementation approximates preemption using execution chunks of maximum duration $\Delta$. Scheduling decisions are made only at chunk boundaries. Therefore, an earlier-deadline job released during a chunk may wait until the current chunk completes.

In Fig.~\ref{fig:chunked_edf}, $J_1^1$ executes a chunk from $t=18$ ms to $t=22$ ms. Job $J_2^2$ is released at $t=20$ ms, but the scheduler rechecks the ready jobs only at $t=22$ ms. Since $J_2^2$ has the earlier absolute deadline, it is selected at the next chunk boundary.
...
The resulting preemption delay is $\delta=2$ ms. In general, $0 \leq \delta < \Delta$.

Let $\mathcal{R}$ denote the set of ready jobs. Algorithm~\ref{alg:chunked_edf} summarizes the implemented Chunked EDF logic, where job selection is repeated after each execution chunk.
...
\begin{algorithm}[!t]
	...
	\begin{algorithmic}[1]
		...
		\State $J_i^j \gets
		\arg\min\limits_{J_k^l \in \mathcal{R}} d_k^l$
